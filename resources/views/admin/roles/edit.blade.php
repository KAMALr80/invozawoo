...
@section('content')
    ...
    <div class="edit-container">
        ...
        {{-- Form --}}
        <form action="{{ route('admin.roles.update', $role) }}" method="POST" class="form-card">
            @csrf
            @method('PUT')
            ...
            <div class="form-group">
                ...
                <textarea id="description" name="description" class="form-textarea @error('description') error @enderror"
                    placeholder="Enter a brief description of this role...">{{ old('description', $role->description) }}</textarea>
                @error('description')
                    <span class="error-message">{{ $message }}</span>
                @enderror
            </div>
